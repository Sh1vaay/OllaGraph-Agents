...
Your proposed responses, code, and SQL queries will be audited by the Critic agent. If the Critic requests adjustments or points out logical errors, correct your code/response in your next turn.
...
Output 'TERMINATE' when a complete answer has been provided.""",
    ...
    critic = AssistantAgent(
       name="Critic",
       llm_config=llm_config_heavy,
       system_message="""You are a Critic / Code Auditor agent.
Your sole job is to review the code blocks (SQL, Python), logic, schemas, and answers generated by the Retriever (Analyst).
Inspect the Retriever's response for:
1. SQL query errors, invalid column/table names, or non-read-only commands.
2. Python code syntax errors, missing packages, import errors, or incorrect file/chart path assumptions.
3. Logical inconsistencies or calculation errors.

If you find any bugs or potential improvements, describe them clearly and ask the Retriever to correct them.
If the code, logic, or answer is correct, functional, and ready to be delivered to the user, you MUST explicitly write "APPROVED" in your response.
Do not output anything else if you approve.""",
       description="Critic / Auditor Agent"
    )
...
async def run_conversation(message: cl.Message):
    ...
    if intent == "CONVERSATIONAL":
        ...
        active_agents = [user_proxy, retriever, critic]
    ...
    def state_transition(last_speaker: autogen.Agent, groupchat: autogen.GroupChat) -> autogen.Agent | None:
        ...
        if last_speaker is user_proxy:
            # User proxy has initiated conversation or completed a tool run.
            # Return to Retriever to analyze outputs.
            return retriever if retriever in groupchat.agents else chatter
            
        if last_speaker is retriever:
            # Retriever (Analyst) has responded. Route to Critic for audit if present.
            if critic in groupchat.agents:
                return critic
            return user_proxy
            
        if last_speaker is critic:
            # Critic has audited. If approved, return to user_proxy to execute/terminate.
            # If rejected, return to retriever to correct.
            if "APPROVED" in last_content:
                return user_proxy
            else:
                return retriever
                
        if last_speaker is chatter:
            return user_proxy
